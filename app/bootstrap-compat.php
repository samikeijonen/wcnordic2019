...
add_action( 'after_switch_theme', 'wc_nordic_2019_switch_theme' );
add_action( 'load-customize.php', 'wc_nordic_2019_load_customize' );
add_action( 'template_redirect', 'wc_nordic_2019_preview' );
...
function wc_nordic_2019_compat_message() {
	if ( version_compare( $GLOBALS['wp_version'], '4.9', '<' ) ) {
		...
		return sprintf(
			...
			esc_html__( 'Theme requires at least PHP version %1$s. You are running version %2$s. Please upgrade and try again.', 'wc-nordic-2019' ),
			'5.6',
			PHP_VERSION
		);
	}
 ...
 * @return void
 */
function wc_nordic_2019_switch_theme( $old_name ) {

	switch_theme( $old_name ? $old_name : WP_DEFAULT_THEME );

	unset( $_GET['activated'] );

	add_action( 'admin_notices', 'wc_nordic_2019_upgrade_notice' );
}
 ...
 * @return void
 */
function wc_nordic_2019_upgrade_notice() {
	printf( '<div class="error"><p>%s</p></div>', esc_html( wc_nordic_2019_compat_message() ) );
}
 ...
 * @return void
 */
function wc_nordic_2019_load_customize() {
	wp_die( esc_html( wc_nordic_2019_compat_message() ), '', array( 'back_link' => true ) );
}
 ...
 * @return void
 */
function wc_nordic_2019_preview() {
	if ( isset( $_GET['preview'] ) ) {
		wp_die( esc_html( wc_nordic_2019_compat_message() ) );
	}
}
